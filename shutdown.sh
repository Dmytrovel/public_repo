...
# Log file where the messages generated by this script will be stored
...
while true; do
    ...
    for ((i=0; i<30; i++)); do
        ...
        if (( $(echo "$cpu_idle < $threshold" | bc -l) )); then
            echo "CPU usage has gone above 3.1%, resetting the cycle." | tee -a $log_file
            break
        fi
    ...
    if (( $(echo "$cpu_idle >= $threshold" | bc -l) )) && [ $change_detected -eq 0 ]; then
        echo "CPU usage remained below 3.1% and no file changes detected for half an hour, shutting down..." | tee -a $log_file
        sudo shutdown -h now
    ...
    echo "No shutdown triggered, starting a new cycle..."  | tee -a $log_file
done
